...
namespace CustomVision.Parser
{
    public class CustomVisionOutputParser
    {
        ...
        public const int CLASS_COUNT = 1;   //模型label 的数量
        ...
        {
            labels = File.ReadAllLines(filepath);
        }
        ...
        public IList<CustomVisionBoundingBox> ParseOutputs(float[] yoloModelOutputs, float threshold = .3F)
        {
            ...
            for (int row = 0; row < ROW_COUNT; row++)
            {
                for (int column = 0; column < COL_COUNT; column++)
                {
                    ...
                    {
                        var channel = (box * (labels.Length + BOX_INFO_FEATURE_COUNT));
