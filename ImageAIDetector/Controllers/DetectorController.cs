...
namespace ImageAIDetector.Controllers
{
    ...
    public class DetectorController : Controller
    {
        ...
        [HttpPost]
        [Route("analysisdemo")]
        public async Task<IActionResult> AnalysisDemo(List<IFormFile> files)
        {
            ...
            foreach (var formFile in files)
            {
                if (formFile.Length > 0)
                {
                    ...
                    using (var stream = System.IO.File.Create(filePath))
                    {
                        ...
                        var data = ProcessDetector(formFile.Name, stream);
                        if(data != null)
                        {
        ...
        [HttpPost]
        [Route("analysis")]
        public async Task<IActionResult> Analysis(List<IFormFile> files)
        {
            long size = files.Sum(f => f.Length);
            ...
            foreach (var formFile in files)
            {
                if (formFile.Length > 0)
                {
                    ...
                    {
                        await formFile.CopyToAsync(stream);
                        var data = ProcessDetectorResult(formFile.Name, stream);
                        if (data != null)
                        {
                            return Ok(data);
                        }
        ...
        private Rectangle? ProcessDetectorResult(string name, FileStream stream)
        {
            Bitmap testImage;
        ...
        private byte[] ProcessDetector(string name,FileStream stream)
        {
            Bitmap testImage;
            var context = new MLContext();
